...
|---|---|---|
| `0` writeset | `{data_dir}/{id}/app.db` | Customer handler `kv.*` via `TrackedTxn` + writeset; `_deploy/current` release marker; the `webhook.send` / `email.send` JS-shim's `_send/owed/{id}` markers ride here too (ordinary kv writes — no apply-time special-case after `effect-reification-plan.md` Phase 5 lands) |
