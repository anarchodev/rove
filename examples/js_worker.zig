...
/// Apply --dev mode sane defaults to the routing-sensitive Cli fields
/// that are otherwise null. Operator can override any of these by
/// passing the explicit flag — the default only kicks in when the
/// corresponding cli field is null after parsing.
///
/// Defaults applied:
///   --admin-api-domain  → "admin.localhost"
///   --public-suffix     → "dev.localhost"
///   --admin-origin      → "https://<admin-api-domain>:<port-from-http>"
///   --bootstrap-root-token → read or generate at
///                        <loop46-data-dir>/dev-root-token
///
/// Both `admin.localhost` and `*.dev.localhost` resolve to loopback
/// per RFC 6761 (any subdomain of `localhost` is reserved for it),
/// so no /etc/hosts edits are needed. The cert generated by
/// `initDevTls` covers `admin.localhost` explicitly and customer
/// instances under `*.dev.localhost` — see the SAN-list comment in
/// initDevTls for why the customer wildcard is 3-label deep.
fn applyDevDefaults(allocator: std.mem.Allocator, cli: *Cli) !void {
    if (!cli.dev) return;

    if (cli.admin_api_domain == null) {
        cli.admin_api_domain = "admin.localhost";
    }
    if (cli.public_suffix == null) {
        cli.public_suffix = "dev.localhost";
    }
    if (cli.admin_origin == null) {
...
fn initDevTls(allocator: std.mem.Allocator, paths: anytype) !void {
    ...
    //
    // Wildcard depth caveat: OpenSSL 3.x (and many modern TLS stacks)
    // reject wildcard certs whose wildcard sits directly under a TLD-
    // like label, so `*.localhost` and `*.test` won't validate against
    // `admin.localhost` / `admin.test` in curl + most clients. We work
    // around it two ways:
    //   - `admin.localhost` is listed explicitly (no wildcard needed).
    //   - Customer instances live under `*.dev.localhost` (3-label
    //     wildcard — OpenSSL accepts), surfaced via `--public-suffix
    //     dev.localhost` in the --dev defaults below.
    // RFC 6761 still gives us "any subdomain of `localhost` resolves to
    // loopback" so no /etc/hosts edits are needed.
    {
        const argv = [_][]const u8{
            "mkcert",
            "-cert-file", paths.cert,
            "-key-file",  paths.key,
            "admin.localhost", "*.dev.localhost",
            "*.test", "test",
            "*.api.test", "api.test",
            "localhost",
            "127.0.0.1",
...
pub fn main() !void {
    ...
    // --dev mode: apply sane defaults to optional routing fields +
    // generate-or-read a persistent root token. Operator's explicit
    // flags always win.
    applyDevDefaults(allocator, &cli) catch |err| {
        std.debug.print("error: --dev defaults: {s}\n", .{@errorName(err)});
        std.process.exit(2);
    };
    if (cli.dev) {
