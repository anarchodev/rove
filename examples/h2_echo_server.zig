...
//! The HTTP/2 echo server, in the declared-world shape: the root module
//! declares the program's world ONCE (`rove_world`), built from h2's
//! parts — which fold io's — and `MyH2.Reg` resolves to the world's
//! registry, which OWNS every declared collection; consumers address
//! them through `reg.coll(.name)` in the world's one namespace. The one
//! ending contract: terminal stream entities are ended through
//! `server.destroyEntity` — the funnel whose dead-letter reaper frees
//! the h2-owned buffers. Conn foreign state (nghttp2 session, TLS) is
//! reaped by h2's `conn_dead` phase.
