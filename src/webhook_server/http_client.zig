...
/// **TEST-ONLY** escape hatch that accepts `http://` webhook URLs
/// in addition to `https://`. **Never set in production** —
/// plaintext delivery leaks request bodies on any intermediate hop.
/// Paired with `ssrf.test_allow_loopback`; both flip together under
/// the worker's `--dev-webhook-unsafe` CLI flag.
...
pub fn deliver(allocator: std.mem.Allocator, req: Request) DeliverError!Response {
    ...
    if (!is_https and !(is_http and test_allow_plaintext)) return DeliverError.HttpsRequired;
