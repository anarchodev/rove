...
// engine-pinned per-chain identity: request.tenant + request.sagaId).
...
export function onMessage({ next, stream, after }) {
  const issued = browser.getReplay({ after }, { on: "onReplay" });
  if (!issued) {
