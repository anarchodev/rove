...
// branch keyed on request.tenant + request.sagaId (browser.getReplay) was
// undriveable — the sim never set either, so getReplay always returned false. Now
// scenario({tenant, sagaId}) supplies them and every activation (WS frame,
// fetch resume) inherits the chain's identity, so BOTH branches are reachable.
import { scenario, expect } from "rewind:test";

// ── issued branch: tenant + sagaId present → getReplay fetches ──
const s = scenario({ tenant: "acme", sagaId: "corr-1", now: "2026-07-01T00:00:00Z" });
...
expect(fetched.url).toContain("tag._saga=corr-1");
...
// ── unavailable branch: no sagaId → getReplay can't issue ──
