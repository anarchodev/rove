...
// a `schedule({ kv }, )` emitter. Tested standalone via scenario().wake(), the same way
...
export default function ({ kv }) {
  ...
  if (n < 3) {
    schedule({ kv }, { in: "1d" }, "jobs/reminder", { user, count: n }, { key: "reminder/" + user });
  }
  return { ok: true, count: n };
}

// A NAMED export — the target of a `schedule({ kv }, ..., "jobs/reminder.mjs.weekly")`.
