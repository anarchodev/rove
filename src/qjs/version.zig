...
/// GC arena regime (arenajs `JS_ARENA_REQ_MODE_GC`). Every current
/// engine runs GC only (`snap.zig`; `decisions.md` §4.12), so the
/// dispatcher sets this bit on every record — the allocator regime is
/// part of the execution identity a replayer must reproduce (a request
/// whose live set fits GC's peak ceiling can OOM under a cumulative
/// bump ceiling), and a replayer that still branches on the bit picks
/// GC. Records from before the regime was fixed carry it unset (they
/// ran under bump) and replay as they ran. Riding the existing word
/// keeps the frozen readset header / log-batch / bundle formats
/// untouched. Bits 0–14 remain the monotonic engine version; mask with
