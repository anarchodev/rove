...
function _rejectRenamed(verb, opts, renames) {
  ...
  globalThis.after = {
     ...
     * @param {boolean} [opts.relay=false] - CAS→connection relay: for a
     *   streamed read of the platform's content-addressed stores (a
     *   `blob.url`-style engine door), intermediate chunks are spliced
     *   straight onto the held stream — `{on}` fires only for the first
     *   event and the terminal. Inert on any other fetch.
