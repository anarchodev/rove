...
pub const DispatchState = struct {
    ...
    /// The host promise whose settlement started this activation
    /// (`held.zig` — set by the dispatcher's resumeHeld entry for fetch
    /// and input settles; null for wake batches, whose per-entry choice
    /// rides the batch JSON). Consumed by `finishResponse`, which stamps
    /// it as the reserved `_settled` record tag AFTER the handler ran —
    /// same posture as `parent_saga`/`_parent` above.
    settled_promise: ?u32 = null,
