...
/// (handler-surface Phase 3, `docs/handler-shape.md` §2.3). `opts.on`
/// selects the bound export — the SAME key the customer writes on
/// `after.fetch` (the shim passes it through; no wire respelling).
/// Issues an
