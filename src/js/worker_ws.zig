...
fn finishWsResume(
    ...
    switch (oc.*) {
        .terminal => |*r| {
            ...
            const lh = worker_streaming.fireLogHeader(p.request_id, tc.snap.deployment_id, @intCast(@max(@min(r.status, 599), 100)), act, rl.method, rl.path, rl.host, chain_ctx.saga_id, p.now_ns, p.exec_seq);
            ...
            captureLogWithId(worker, chain_ctx.tenant_id, p.request_id, rl.method, rl.path, rl.host, tc.snap.deployment_id, p.now_ns, st, .ok, r.console, r.exception, tapes, chain_ctx.saga_id, r.tags, act, fw_seq, p.exec_seq);
            r.console = &.{};
            r.exception = &.{};
            tearDownWsChain(worker, conn_ent);
        },
        .continuation => |*cval| {
            ...
            captureLogWithId(worker, chain_ctx.tenant_id, p.request_id, rl.method, rl.path, rl.host, tc.snap.deployment_id, p.now_ns, 200, .ok, &.{}, &.{}, tapes, chain_ctx.saga_id, cval.tags, act, fw_seq, p.exec_seq);
...
fn fireWsMessage(
    ...
    const run_oc = worker_mod.runResume(worker, p.dep.inst, tc, p.dep.bc, p.txn, &p.ws, request, &budget, path) catch {
        ...
        p.txn_done = true;
        captureLogWithId(worker, chain_ctx.tenant_id, p.request_id, rl.method, rl.path, rl.host, tc.snap.deployment_id, p.now_ns, 500, .handler_error, &.{}, &.{}, .{}, chain_ctx.saga_id, &.{}, .ws_message, 0, p.exec_seq);
        effect_mod.cmd.emitWsSend(worker, .{ .conn_entity = conn_ent, .opcode = 8, .bytes = &.{} }) catch {};
        tearDownWsChain(worker, conn_ent);
        return;
    };
