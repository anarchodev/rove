...
pub const Config = struct {
    ...
    // V2: the `cluster: ?*kv.Cluster` field was removed with the files-
    // server's retired willemt-raft path. The files-server no longer
    // proposes manifests through raft — manifests live in the per-tenant
    // `deployments/` BlobBackend and the `_deploy/current` flip is the
    // worker's `/_system/release` (envelope-0 via the per-tenant bridge).
};
...
fn handleOne(
    ...
    //
    // V2 (branch `v2`): the files-server's OWN willemt-raft cluster is
    // retired — deployment manifests live in the per-tenant `deployments/`
    // BlobBackend (shared fs/s3 with the worker) and the `_deploy/current`
    // flip is the WORKER's `/_system/release` (envelope-0 through the
    // per-tenant bridge), not the files-server's old `cluster.proposeAndWait`.
    // So the files-server is a pure compile + blob-write service now; the
    // cluster-coupled routes (`/_system/cluster-put|cluster-get|manifests/
    // batch`, `deployments/{id}/manifest.bin`) and the `Config.cluster` field
    // were excised with the V1 raft path. `/_system/leader` stays as a
    // single-instance 501 so any tool that probes it still gets a defined
    // answer ("no raft here; this instance accepts all writes").
