...
- **Transport.** Engine thread → per-worker `RelayInbox` (`msg_router.zig`) →
  ...
  every Msg is an activation, and a relay byte-run is precisely not one.
- **Backpressure is lossless, end-to-end.** The engine bounds
  relayed-but-unconsumed bytes per transfer (`RELAY_WINDOW_BYTES`); at the cap
  the curl write callback returns `CURL_WRITEFUNC_PAUSE` (bytes held +
  re-delivered, never dropped). The worker consume-acks appended bytes
  (`FetchEngine.ackRelay`); the engine thread unpauses at half-window. The
  worker side feeds the stream only under `StreamChunks`' soft cap, so the h2
  egress pacing is unchanged. Relay transfers run untimed (client-paced
  lifetime); a client disconnect cancels the fetch via the held chain's
  teardown, which is also what unsticks a paused transfer whose consumer died.
- **Failure posture.** Any path that cannot both keep and order the bytes kills
  the transfer (abort → `CURLE_WRITE_ERROR` → terminal `ok=false`) — never a
  silent drop under a committed 200 (the truncated-asset bug class).
- **Recording.** Relayed bytes produce no record and no tape: they are
  immutable, content-addressed, and reproducible from the hash the two
  remaining records reference (never-tape-blobs).
