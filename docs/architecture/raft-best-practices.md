...
| **`propose_conf_change` / learners / `voter_progress`** | membership changes through the native conf-change path — add voters one at a time, join new nodes as learners and promote after catch-up (never shrinks the quorum). A learner bootstraps its tenant store **out-of-band** (the tenant-move bundle, off the leader's hot path), then joins raft already caught up. The FFI methods now live in `bridge.zig`; CP node-join orchestration (out-of-band bootstrap → learner-join → catch-up → promote) is tracked by `cp-membership-reconciler.md`. | fork conf-change extern fns; `bridge.zig` `proposeConfChange` / `asLearner` / `voterProgress`; see `cp-membership-reconciler.md` |
