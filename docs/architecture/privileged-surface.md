...
## 3.5 Privileged reads are recorded inputs
...
- **Recording cross-store reads puts other tenants' values, and platform root
  ...
  *credential*, which is stricter and goes the other way: data the handler
  legitimately consumes gets recorded, whereas a credential it should never
  hold is removed from the surface instead of taped.
