...
- Plan tiers wired to all per-account caps via the `account/{sha256(email)}/plan` namespace introduced in Phase 4. Single lookup drives `max_instances` (Phase 4 hardcode goes away), `request` / `email` / `magic_resend` rate-limit caps, DLQ retention, blob storage caps, custom-domain counts, and Stripe customer-id linkage below.
...
- **NOT yet — lazy-creation-on-redeem + admin-handler port to JS (2026-04-29).** Two coupled amendments folded into one work item — they touch the same handlers, and porting Zig that's about to be rewritten anyway is wasted churn. **(a) Lazy creation:** today an unredeemed signup permanently squats the name and leaks `{id}/app.db`, `{id}/files.db`, the file-blob directory, and an outbox row. Fix: signup writes only `pending/{name}` → `{email, expires_at_ns, resend_grace_until_ns, magic_hash_hex}` in admin app.db; duplicate check covers both `instance/*` and non-expired `pending/*`; outbox row for the magic email moves to admin app.db; redemption is where instance creation, root_writeset propose, starter deploy, files_writeset propose, `pending/*` cleanup, and session mint all happen. Periodic sweep over `pending/*` + `magic/*` clears entries past their resend grace window. **(b) Zig→JS handler port:** §10.1 made admin a real instance with its own app.db and `platform.root.*`; this extends the same dogfooding to the *handlers*. `/v1/signup`, `/v1/auth`, `/v1/login`, `/v1/logout`, `/v1/session` stop having dedicated Zig handlers in `worker.zig` and route to admin's deployed JS handler like any other path on `app.loop46.me`. Routing fast path stays Zig (host→tenant unchanged). Three new primitives: `platform.instances.create(name)` / `.delete(name)` (admin-only, wraps `tenant.createInstance` + `proposeRootWriteSet`), plus `crypto.randomBytes(n)` and `crypto.sha256(data)` extending the existing `crypto.hmacSha256` namespace. With those, mint/redeem/session become trivial JS using `kv.*` against admin's own app.db; `handleAdminSignup`/`handleAdminAuth`/`handleAdminLogin`/`handleAdminLogout`/`handleAdminSession`/`requireAdminAuth`/`deployStarterContent`/`queueSignupEmail`/`buildSignupEmailText` in `worker.zig` and `mintMagic`/`redeemMagic`/`createSession`/`authenticateSession` in `tenant/root.zig` all delete. Starter content deploys via the existing two-phase deploy primitive — admin JS calls it like any deploy client, no special starter knob. Detailed flow in [`docs/flows/signup.md`](flows/signup.md).
- **NOT yet — auto-resend on expired-but-recent magic click (2026-04-29).** Magic and pending records carry a `resend_grace_until_ns = expires_at_ns + 15min`. Within that grace window, clicking an expired link triggers a rate-limited resend (new `magic_resend` action: per-email bucket ~3/hr against email-bombing, plus a fleet-wide bucket against bot click-storms). Old magic is deleted on resend so the link is one-shot; subsequent clicks return 401 and the user reads their email for the fresh link. Outside the grace window, behavior matches today's (401, user re-signs-up).
- **NOT yet — per-account instance limit (2026-04-29).** Email is the account identity. New `account/{sha256(email)}/...` namespace in admin app.db owns plan tier + instance index. Signup-time check counts both `instance/*` ownership and non-expired `pending/*` reservations against `max_instances` (otherwise free tier is evadable via name squatting); redeem-time recheck is the authoritative gate. v1 hardcodes `max_instances = 1` for free; Phase 10 wires the value from `account/{hash}/plan`. Seed-manifest tenants are operator-provisioned and stay outside the account model. Same `account/{hash}/plan` lookup becomes the seam for all Phase 10 per-account caps (rate limits, DLQ retention, blob caps, custom-domain counts, Stripe linkage).
