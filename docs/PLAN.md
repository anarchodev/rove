...
- (d) **webhook subsystem step 1 — done 2026-05-05.** New module `rove-webhook-server` (`src/webhook_server/`) ships the schema, wire format, and apply primitives for the cluster-wide `webhooks.db`: `WebhookRow` / `WebhookStore` with `applyEnqueueBatch` / `applyComplete` / `applyRetrySchedule`, plus encode/decode for envelope types 4 / 5 / 6. Step-1 storage uses a kv-backed wrapper (rows under `wh/{webhook_id_hex}`); the real partial-index schema lands in step 2 when the delivery loop needs it. Apply-path integration, leader-pinned delivery thread, and dispatcher cutover are the remaining steps — see `docs/phase-5.5-rollout.md` §2.
