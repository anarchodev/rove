...
2. **Stripe credibility example tenant + docs** (per §10.14 "customer third-party auth toolkit") — addresses Story 2's "real work, not toys" doubt. Real flow: paid feature gate, Stripe Elements for card collection, webhook handler with timing-safe HMAC verify, customer dashboard showing subscription status. Demonstrable artifact. ~1 week, plus a small primitives audit (`crypto.timingSafeEqual` confirmed; `base64Encode/Decode` and `hmacSha1` if needed).
...
**Total launch path**: ~9 weeks if serialized, less if parallelized. ~2-2.5 months from this decision.
