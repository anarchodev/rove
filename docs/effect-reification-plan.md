...
> **Revision (2026-05-25):** PR-1 originally introduced
> `Cmd.blob_put` as a Zig primitive. Revised to ship `blob.put.js` /
> `blob.get.js` as JS shims composing `kv.set("_blob/owed/{hash}",
> marker) + http.fetch PUT` — same composition discipline as Phase 5's
> `webhook.send.js`. Rationale + recovery story:
> `effect-algebra.md` §2.5 + §6 rule 4 + §7 worklist item 8;
> customer-facing surface: `primitive-gaps.md` §7.4. PR-2 (compile
> worker) keeps its Zig-primitive framing — compile is CPU-bound
> in-process work with no underlying primitive to shim over.
...
(`files-server-standalone`) as a JS-shim composition over **one new
primitive** (an in-process compile worker), **two new JS shims**
(`blob.put.js` / `blob.get.js`, composing existing primitives), kv,
and the existing admin auth path. PLAN §13's live process map loses
...
BlobBackend — composable from `kv.set` (marker) + `http.fetch` (PUT to
the backend's URL); (b) **QuickJS compile** — a Cmd runtime, producing
a `compile_done` Msg origin; no underlying primitive to compose from;
...
no allocator is needed. (a) + (c) + (d) are already covered by
existing primitives + standard-library shims; only (b) becomes a
new Zig primitive. The whole service composes in JS.
...
admin tenant JS (e.g. globals/files.js or __system/files/upload.mjs):
  ...
  // upload — compile.queue is the new Cmd; blob.put is a JS shim
  // (kv.set "_blob/owed/{hash}" marker + http.fetch PUT)
  const cb = await compile.queue(source);             // Cmd → compile_done Msg
  await blob.put(`blobs/${cb.bytecode_hash}`, cb.bytecode);
                                                      // shim → http_done Msg
  await blob.put(`blobs/${source_hash}`, source);     // shim → http_done Msg

  // deploy — every blob.put marker rides the activation's writeset;
  // the PUTs themselves fire post-commit per L4 (idempotent via
  // content-addressing, so retry-on-failure is safe).
  const manifest = buildManifest(entries);
  const dep_id = computeDeploymentId(entries);        // content hash
  await blob.put(`deployments/${pad20(dep_id)}.json`,
                  JSON.stringify(manifest));          // shim, idempotent

  // release — ordinary kv write; commits atomically with any
  // _blob/owed/* markers written by the blob.put shims above.
...
**One new primitive + two new JS shims.**

1. **`blob.put.js` + `blob.get.js` (JS shims, not Zig Cmds).**
   `blob.put.js` composes `kv.set("_blob/owed/{hash}", marker)` —
   marker carries `{correlation_id, seq, call_index, dest_key}`, NOT
   the bytes — plus `http.fetch(blob_url(hash), {method: PUT, body:
   bytes})`. Marker rides the activation's writeset (envelope-0,
   atomic with the manifest write). On `http_done` success: shim
   clears the marker in a follow-up activation. Recovery for
   in-flight crashes uses the existing retry-cron + boot-sub
   plumbing from Phase 5 — but because the marker doesn't carry the
   bytes, recovery re-executes the source activation against its
   recorded readset (`readset-replication-plan.md` §4) to re-derive
   the bytes, then re-issues the PUT (idempotent via
   content-addressing). See `effect-algebra.md` §2.5 for the
   "inputs durable / outputs derivable" frame this depends on, and
   `primitive-gaps.md` §7.4 for the customer-facing surface.

   `blob.get.js` is a direct `http.fetch(blob_url(hash))` shim with
   the same response-disposition options as outbound HTTP (whole /
   `on_chunk` / `pipe_to`). No marker, no durability — failed read =
   caller retries. A local-BlobBackend fast-path (skip the HTTP
   round-trip; read from fs directly) is a shim-internal
   optimization that doesn't change the algebra.

   **Pre-readset-replication degradation.** Until
   `readset-replication-plan.md` ships, re-execution recovery has
   no readset to consult. `blob.put.js` ships with
   at-most-once-with-transient-retry semantics in the interim
   (worker crash mid-PUT = customer must re-issue). This matches
   the delivery class of the pre-revision `Cmd.blob_put` framing
   exactly — same customer-visible semantics, simpler engine
   surface, cleaner recovery story unlocks when readset-replication
   lands.

2. **`compile.queue` Cmd + `compile_done` Msg origin** — the
   genuinely new primitive. Compile is CPU-heavy (~5–20ms per 20KB
   `.mjs`, bursting on deploy), in-process work with no underlying
   primitive to shim over. A dedicated compile thread pool
   (size = `min(N_cores - N_workers, 4)`, measured) drains a
   `compile_pending` rove collection; each entry holds
   `{tenant_id, correlation_id, source_bytes, deps[]}`. Worker
...
- `BlobBackend` (`src/blob/`) — unchanged. Stays the destination
  for `blob.put.js`'s PUT and `blob.get.js`'s GET; the shim hits it
  via `http.fetch` against the backend's URL (S3 via SigV4, or
  loopback for fs backends), not via a dedicated Zig Cmd routing
  path.
...
- PLAN §13 process map — loses the `files-server-standalone` row.
  `loop46` now also owns: blob writes (via `blob.put.js` shim
  composing kv + outbound HTTP), QJS compile worker, `__system/files/*`
  handler modules.
...
- **PR-1a — Generalize the durability-marker sweep.** Phase 5's
  retry-cron + boot-sub today scan `_send/owed/*` specifically.
  Refactor to scan a registered set of prefixes (`_send/owed/`,
  `_blob/owed/`, future `_*/owed/`) each pointing at a per-prefix
  callback that knows how to retry that verb's markers. This is a
  small refactor on the Phase 5 sweep code, not a new piece of
  infrastructure; it just stops baking the `_send/owed/` literal in.
  Lands before PR-1b so blob.put markers have something to pick
  them up. ~50–80 LOC. Smoke: re-run `http_send_smoke.py` + any
  Phase 5 sweep tests unchanged.
- **PR-1b — `blob.put.js` + `blob.get.js` shims** (revised
  2026-05-25; previously `Cmd.blob_put` Zig primitive). Ship the
  two standard-library JS shims under `globals/` (or
  `__system/blob/` if scoped to builtin modules — pick at
  implementation time, mirrors Phase 5's `webhook.send.js`
  placement). Marker key shape: `_blob/owed/{hash} =
  {correlation_id, seq, call_index, dest_key, attempts, ts}`. Wire
  `blob_url(hash)` to a per-tenant URL resolver that knows the
  configured BlobBackend (S3 endpoint + SigV4 signing for S3, or
  loopback for fs). Register `_blob/owed/` with the PR-1a
  generalized sweep, providing a callback that re-issues the PUT
  (transient retry today; re-execution recovery once
  readset-replication ships — see below). Smoke:
  `blob_put_smoke.py` — happy path (put then get), idempotent
  re-put (same hash, no duplicate write), marker-cleared-on-success,
  transient-retry on injected network fault. **No `effect/cmd.zig`
  change, no `interpretCmd` wiring, no `__rove_blob_put` Zig
  binding** — that shape is the one PR-1 originally introduced; the
  shim composition doesn't need it. ~200 LOC of JS + small Zig
  wiring delta (per-tenant URL resolver).
- **PR-1c — Recovery-via-re-execution.** Gated on
  `readset-replication-plan.md` landing. Extends the `_blob/owed/`
  callback registered in PR-1b: when a marker has been stale past
  N attempts, load the source activation's readset from raft,
  re-execute the handler in sandboxed mode, walk to the matching
  `blob.put` call by `call_index`, extract bytes, re-issue PUT.
  May land as part of readset-replication's own rollout rather
  than a separate PR — sequence decided when readset-replication
  phases finalize.
...
- **Multi-node compile routing.** Compile runs on the leader only?
  ...
  content-addressed blob, and `blob.put.js`'s marker write is the
  only durable side-effect (idempotent via content-hash addressing).
  A follower can compile; the resulting `blob.put.js` shim writes
  its `_blob/owed/{hash}` marker into the activation's writeset,
  which routes to the leader at commit time like any other writeset.
  The PUT itself fires post-commit per L4, against the configured
  BlobBackend (which is shared across all nodes). PR-2 doesn't need
  leader-pinning.
...
- `http_send_smoke.py` — must pass after PR-1a (generalized sweep
  must remain compatible with the existing `_send/owed/*` semantics).
- NEW: `blob_put_smoke.py` (PR-1b) — happy path (put + get +
  marker-cleared), idempotent re-put (content-addressed dedupe),
  transient-retry on injected network fault. When PR-1c lands,
  extend with crash-mid-PUT scenarios that exercise re-execution
  recovery.
- NEW: `compile_smoke.py` (PR-2) — happy + syntax-error +
  dep-resolution + timeout + concurrent-deploy (idempotent).
...
**Rollback:** PR-1a through PR-4 are additive — revert in reverse
order restores byte-identical behavior since the old service is
still running. (PR-1a's generalized sweep is itself additive —
worst case a revert restores the literal `_send/owed/` scan with
no semantic change.) PR-5 is the irreversible flip; gate it
behind a week of PR-4 stability + a successful deploy-storm bench
run.

**Relation to other plans.**

- **`readset-replication-plan.md`** — overlaps at `BlobBackend`,
  plays opposite roles. Readset replication introduces a
  *per-tenant streaming-append* runtime (§4.4): bytes accumulate
  in an in-memory buffer keyed by `(batch_id, offset, len)`,
  flush advances `durable_offset`, gated callbacks resume. That
  path is the durable home for **input bytes** (request bodies,
  fetch responses) — losing them = losing inputs (unrecoverable).
  Phase 7's `blob.put.js` writes **output bytes** (handler-produced,
  content-addressed) to the same backend at a different path
  (`{tenant}/blobs/{hash}` vs `{tenant}/readset-blobs/{batch_id}`).
  Output bytes are a cache: transiently lost bytes are
  recoverable by re-executing the source activation against the
  recorded readset (`effect-algebra.md` §2.5). The two namespaces
  share infrastructure but have opposite loss semantics. The
  shared per-tenant batcher abstraction (which readset-replication
  shares with `logs-plan.md`) sits beneath both, not part of
  Phase 7's surface.

  **Dependency:** `blob.put.js`'s full recovery story
  (re-execute-to-re-derive bytes) requires readset-replication's
  readset to be in raft entries. Until readset-replication ships,
  `blob.put.js` ships with at-most-once-with-transient-retry
  semantics; the cleaner recovery path turns on later. See PR-1a
  / PR-1b / PR-1c above for the staged rollout — PR-1c is the
  recovery-via-re-execution piece, gated on readset-replication
  landing.
...
- **`effect-algebra.md` §2.4 + §2.5 + §6 rule 4** — Phase 7 adds
  one member to the Cmd-runtimes open family (`compile.queue`) and
  one to the Msg-origins open family (`compile_done`). `blob.put`
  and `blob.get` are NOT new primitives — they're JS-shim
  compositions of existing primitives (kv + outbound HTTP)
  following the same pattern as `webhook.send.js` / `email.send.js`
  from Phase 5. The §2.5 "inputs durable / outputs derivable"
  corollary is what makes the marker-without-body recovery story
  work for arbitrary-size blob payloads. All Phase 7 effects obey
  L3/L4 by construction (existing reified ingress + commit-gated
  buffer Phase 4.1 generalized). The algebra grows by exactly one
  predicted axis (`compile.queue`/`compile_done`); the blob surface
  fits without growth.
