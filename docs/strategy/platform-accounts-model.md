...
> the users-lib plan (issue #95; the *customer*'s end-user auth —
...
| **End-user (customer app)** | the *customer's own users* of *their* deployed app | `users-lib` inside the customer's tenant (issue #95) | **deliberately deferred** — B2C-only, "Orgs/B2B: Out" |
