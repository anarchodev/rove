...
# app, and the cluster root (rove#715: a standing per-cluster group,
# formed by the CP's ensurer at genesis). A leader kill orphans every
# group the victim led, so the acquisition accounting below has to know
# the full set (rove#374) — a group missing here reads as a spurious
# extra election whenever the victim happened to lead it (~1/3 of kills).
...
def main() -> int:
    ...
    with V2Cluster.spawn("failover", nodes=3) as c:
        ...
        # The cluster runs the GROUPS set above, and every group the
        # victim leads is orphaned by the kill and legitimately elects
        # once: the honest expectation is "one acquisition per orphaned
