#!/usr/bin/env python3
"""CP→DP plan delivery smoke (gap #1, step 2; operational-state delivery to
the DP, docs/architecture/control-plane.md).
...
the repo `.env` first:
  `set -a; . ./.env; set +a; python3 scripts/smoke/cp_plan_delivery_smoke.py`
