...
#
#   - ADMIN_ORIGIN  (e.g. https://app.rewindjscom.localhost:8210)
...
# scripts/smoke/*_smoke.sh, the repo root is two levels up. Fall back
...
spawn_files_server() {
    ...
    env "${env_assigns[@]}" \
        ...
        ${leader_url:+--web-root "${ROVE_WEB_ROOT:-$(pwd)/web}"} \
        "${extra_args[@]}" \
        >"$log_path" 2>&1 &
    CS_PID=$!
