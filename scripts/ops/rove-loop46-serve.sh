...
# ~/.rove/tls/{cert,key}.pem (maintained by scripts/ops/rove-lego-renew.sh).
#
# ── Prerequisites ────────────────────────────────────────────────────
# 1. scripts/ops/rove-lego-renew.sh has issued a cert at least once:
...
if [[ ! -r "$TLS_CERT" ]]; then
    ...
    echo "       run scripts/ops/rove-lego-renew.sh first" >&2
    exit 2
fi
if [[ ! -r "$TLS_KEY" ]]; then
