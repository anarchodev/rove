...
#   scripts/ops/phase32_gate.sh            # correctness should pass too
#
#   # 2. after the migration:
#   zig build -Doptimize=ReleaseFast install
#   scripts/ops/phase32_gate.sh            # must still pass, incl. --check
#
...
#       scripts/bench/kv_shard_bench.sh
#
set -uo pipefail
...
echo "  (sharded ceiling NOT gated here — run scripts/bench/kv_shard_bench.sh)"
echo
