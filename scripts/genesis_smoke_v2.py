#!/usr/bin/env python3
"""Genesis smoke ⭐ — bring up a 3-node cluster FROM EMPTY via born-{self}+grow.
...
multi-node bring-up, and the cold-multi formation we shipped (every node born
with the full voter set) couldn't elect on real nodes. The codified fix is
TiKV-style bootstrap-one-then-grow — a group is born as the SOLE voter {1} on
ONE node (auto-leads, no election race) and the CP's membership reconciler
grows it to the full node set learner-first. This smoke spins a REAL
multi-process cluster from empty and asserts that path end to end.

Topology (genesis mode — `V2Cluster.spawn(..., genesis=True)`):

    rewind-cp     single node, reconciler ON (REWIND_CP_RECONCILE_MEMBERSHIP=1)
    rewind-front  stateless proxy
    rewind nodes  ×3, each booted SELF-ONLY (REWIND_NODE_ID + REWIND_RAFT_ADDR,
                  no static REWIND_VOTERS/REWIND_PEERS) — groups born {self}

Legs:
  A.  provision a tenant → the CP births its group as the sole voter {1} on
      node 1 (auto-leads immediately, no cold-multi election).
  B.  the reconciler grows the born-{self} group to all 3 nodes (learner →
      catch-up → promote), riding §4d attach-carry so the joiners can ACK.
  ...
  E.  ⭐ kill the leader; a promoted survivor serves the replicated data and
      accepts a fresh write on the surviving quorum.
...
def main() -> int:
    ...
    with V2Cluster.spawn("genesis", nodes=nodes, genesis=True) as c:
        # ── A. provision births {1} on node 1 ─────────────────────────
        print("leg A: provision (births the group as sole voter {1} on node 1)")
        r = c.provision(TENANT)
        check("provision → 204", r.status == 204, f"got {r.status} {r.body!r}")

        # ── B. the reconciler grows {1} → {1,2,3} ─────────────────────
        print("leg B: reconciler grows the born-{self} group to all 3 nodes")
        try:
            ms = c.wait_for_membership(TENANT, voters=nodes, timeout=60.0)
            check("grew to 3 voters", len(ms.get("voters", [])) == nodes,
                  f"voters={ms.get('voters')}")
        except SystemExit as e:
            check("grew to 3 voters", False, str(e))
            c.dump_node_log(grep=["reconcile", "confchange", "learner", "promote",
                                  "attach", "peer", "error", "warn"])
        ...
        print("leg E: ⭐ kill the TENANT leader; a survivor promotes + serves")
        leader = c.leader_node(TENANT)
        check("found leader to kill", leader is not None, f"node {leader}")
        if leader is not None:
            c.kill_node(leader)
            survivors = [i for i in range(nodes) if i != leader]
            # a promoted survivor still serves the replicated value
            rr = None
            for i in survivors:
    ...
    if failures:
        ...
        return 1
    print("\nPASS — genesis: a 3-node cluster formed FROM EMPTY via born-{self}"
          "+grow, replicated, served a publish, and survived a leader kill. ⭐")
    return 0
